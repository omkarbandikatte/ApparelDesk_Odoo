...
const SaleOrders = () => {
  ...
  const fetchOrders = () => {
    const stored = localStorage.getItem('saleOrders');
    if (stored) {
  ...
  const fetchContacts = () => {
    ...
    if (stored) {
      const parsed = JSON.parse(stored);
      setContacts(parsed.filter(c => c.contactType === 'customer' || c.contactType === 'both'));
    } else {
      setContacts([]);
    }
  };
  ...
  const fetchPaymentTerms = () => {
    ...
    if (stored) {
      setPaymentTerms(JSON.parse(stored));
    } else {
      setPaymentTerms([
  ...
  const generateSONumber = () => {
    ...
    if (stored) {
      ...
      if (lastOrder && lastOrder.soNumber) {
        const match = lastOrder.soNumber.match(/SO(\d+)/);
        if (match) {
  ...
  const loadOrder = (order) => {
    setOrderData({
      ...
      soDate: order.soDate || new Date().toISOString().split('T')[0],
  ...
  const handleConfirm = () => {
    ...
    localStorage.setItem('saleOrders', JSON.stringify(allOrders));
    fetchOrders();
  };
  ...
  const handleCreateInvoice = () => {
    // Navigate to invoices or create invoice
    alert('Create Invoice functionality - will navigate to Invoices');
  };

  const totals = calculateTotals();
  const isNew = currentIndex === -1;
  const customer = contacts.find(c => c.id === parseInt(orderData.customerId));
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container px-6 py-8">
        ...
        <Card className="p-8">
          <h1 className="text-2xl font-bold mb-6">Sale Order</h1>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setOrderData({ ...orderData, soNumber: e.target.value })}
                placeholder="SO0001"
              />
              <p className="text-xs text-muted-foreground">(auto generate SO Number + 1 of last order)</p>
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium">SO Date</label>
              <Input
                type="date"
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="space-y-2">
              ...
              <select
                className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                value={orderData.paymentTermId}
                onChange={(e) => setOrderData({ ...orderData, paymentTermId: e.target.value })}
              >
                      ...
                      <td className="px-4 py-3">
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            if (product) {
                              handleLineChange(index, 'unitPrice', product.salesPrice || 0);
